...
class Tcl_Obj extends ffi.Struct {
  ...
  external ffi.Pointer<ffi.Int8> bytes;

  /// The number of bytes at *bytes, not including the terminating null.
  @ffi.Int32()
  external int length;
}

extension TclObjExtensions on Tcl_Obj {
  String asString({bool allowMalformed = true}) {
    //final ptrName = this.bytes.cast<ffi.Utf8>();
    final ptrNameCodeUnits = this.bytes.cast<ffi.Int8>();
    final list = ptrNameCodeUnits.asTypedList(this.length);
    return utf8.decode(list, allowMalformed: allowMalformed);   
  }
}

// extension PointerTclObjExtensions on ffi.Pointer<Tcl_Obj> {
//   String asString({bool allowMalformed = true}) {
//     final ptrName = this.ref.value.bytes.cast<ffi.Utf8>();
//     final ptrNameCodeUnits = this.bytes.cast<ffi.Uint8>();
